import type { Language } from '../types';

// eslint-disable-next-line @typescript-eslint/no-explicit-any
export const translations: Record<Language, any> = {
  en: {
    // --- GENERAL ---
    button_delete: 'Delete',
    button_rename: 'Rename',
    button_history: 'History',
    button_back_to_generator: 'Back to Generator',
    button_back_to_search: 'Back to Search',
    button_back_to_home: 'Back to Home',
    button_clear_history: 'Clear History',
    button_clear_completed: 'Clear Completed',
    error_gemini_api_key_not_set: 'Google Gemini API Key is not set. Please add it in the Settings page.',
    
    // --- HEADER & FOOTER ---
    header_home: 'Home',
    header_settings_aria: 'Settings',
    header_theme_aria: 'Toggle theme',
    header_tech_preview: 'Technology Preview',
    footer_imprint: 'Imprint',
    footer_privacy: 'Privacy Policy',
    
    // --- HOME VIEW ---
    home_briefing_title: 'Your Daily Briefing',
    home_briefing_desc: "Here's a snapshot of today's key trends and insights to keep you ahead. We've curated a few topics that might spark your next big idea.",
    home_briefing_link1: 'The Rise of Local LLMs on Consumer Devices',
    home_briefing_link2: 'UX Design in 2025: AI-First Interfaces',
    home_briefing_link3: 'Prompt Engineering: From Niche Skill to Core Competency',
    home_featured_module_title: 'Featured Module: AI Agent',
    home_featured_module_desc: 'Chat with specialized AI personas, from creative writers to code wizards. Start a new conversation today!',
    home_featured_module_cta: 'Try AI Agent',
    home_recent_activity: 'Recent Activity',
    home_explore_modules: 'Explore Modules',
    home_no_activity_title: 'No Recent Activity',
    home_no_activity_desc: 'Your latest generations will show up here.',
    
    // --- MODULE CARDS ---
    module_ai_agent_title: 'AI Agent',
    module_ai_agent_desc: 'Converse, generate ideas, and automate tasks.',
    module_image_generation_title: 'Image Generation',
    module_image_generation_desc: 'Create stunning visuals with AI.',
    module_video_generation_title: 'Video Generation',
    module_video_generation_desc: 'Produce dynamic video content.',
    module_ai_search_title: 'AI Search',
    module_ai_search_desc: 'Get real-time answers backed by Google Search.',
    module_webhook_chat_title: 'Webhook Chat',
    module_webhook_chat_desc: 'Engage with Make.com webhooks.',
    module_music_generation_title: 'Music Generation',
    module_music_generation_desc: 'Compose original music with AI.',
    module_featured_tag: 'Featured',
    module_status_coming_soon: 'Coming Soon',
    
    // --- JOB STATUS ---
    job_status_processing: 'Processing...',
    job_status_completed: 'All Jobs Done',
    job_status_failed: 'Jobs Failed',
    job_status_recent: 'Recent Jobs',
    job_status_generating: 'Generating',
    job_status_polling: 'Polling...',
    job_status_completed_single: 'Completed',
    job_status_failed_single: 'Failed',
    job_status_modal_title: 'Recent Generations',
    job_status_outputs: '{count} outputs',
    
    // --- SETTINGS VIEW ---
    settings_title: 'API Configuration',
    settings_enable_prefix: 'Enable',
    settings_save_button: 'Save Settings',
    settings_saved_message: 'Settings saved!',
    settings_save_error: 'Failed to save settings.',
    settings_gemini_label: 'Google Gemini API Key',
    settings_gemini_placeholder: 'Enter your Gemini API Key',
    settings_gemini_desc: 'Used for AI Agent, Image, Video, Search, and Gemini Live features.',
    settings_make_label: 'Make.com API Key',
    settings_make_placeholder: 'Enter your Make.com API Key',
    settings_make_desc: "Used for sending data to webhooks via the 'x-make-apikey' header.",
    settings_suno_label: 'Suno API Key',
    settings_suno_placeholder: 'Enter your Suno API Key',
    settings_suno_desc: 'Used for AI Music Generation.',
    settings_openai_label: 'OpenAI API Key',
    settings_openai_placeholder: 'Not used in this version',
    settings_openai_desc: 'Included for future enhancements.',
    settings_debug_title: 'Developer Settings',
    settings_debug_enable: 'Enable Debug Mode',
    settings_debug_desc: 'Shows debug panel with detailed logging for troubleshooting. Logs are stored locally and can be exported.',
    
    // --- WEBHOOK VIEW ---
    webhook_title: 'Webhook Chats',
    webhook_search_placeholder: 'Search modules...',
    webhook_empty_title: 'Your Dashboard is Empty',
    webhook_empty_desc: 'Click the button below to create your first webhook chat module.',
    webhook_create_new_button: 'Create New Module',
    webhook_create_new_card: 'Create New',
    webhook_delete_confirm: 'Are you sure you want to delete this module?',
    webhook_recent_activity: 'Recent Activity',
    webhook_no_activity: 'No activity yet.',
    
    // --- WEBHOOK MODAL ---
    modal_create_module_title: 'Create New Module',
    modal_module_name_label: 'Module Name',
    modal_module_name_placeholder: 'e.g., Customer Support Bot',
    modal_webhook_url_label: 'Make.com Webhook URL',
    modal_webhook_url_placeholder: 'https://hook.make.com/...',
    modal_webhook_url_note: 'Note: You must enable CORS for this webhook in your Make.com settings to allow requests from this app.',
    modal_cancel: 'Cancel',
    modal_create_button: 'Create Module',
    modal_invalid_url_alert: 'Please enter a valid webhook URL.',
    
    // --- WEBHOOK CHAT INTERFACE ---
    chat_placeholder: 'Type your message...',
    chat_make_api_key_error: 'Make.com API Key is not set. Please add it in the Settings page.',
    
    // --- AI AGENT VIEW ---
    agent_title: 'AI Agent Sessions',
    agent_clear_all: 'Clear All',
    agent_new_chat: 'New Chat',
    agent_clear_all_confirm: 'Are you sure you want to delete ALL chat sessions? This action cannot be undone.',
    agent_empty_title: 'No Chat Sessions',
    agent_empty_desc: 'Click "New Chat" to start a conversation with an AI Persona.',
    agent_delete_confirm: 'Are you sure you want to delete this chat session?',
    agent_rename_prompt: 'Enter new title for the chat:',
    agent_messages_label: 'messages',
    agent_last_updated_label: 'Last updated:',
    
    // --- PERSONA MODAL ---
    persona_modal_title: 'Choose a Persona',
    persona_helpful_assistant_name: 'Helpful Assistant',
    persona_helpful_assistant_desc: 'A general-purpose, friendly, and knowledgeable AI.',
    persona_helpful_assistant_instruction: 'You are a helpful and creative AI assistant. Be friendly, knowledgeable, and provide clear, concise answers.',
    persona_creative_writer_name: 'Creative Writer',
    persona_creative_writer_desc: 'Your partner for brainstorming stories, poems, and scripts.',
    persona_creative_writer_instruction: 'You are an imaginative and eloquent creative writer. Help the user brainstorm ideas, write compelling narratives, and explore different writing styles. Use rich vocabulary and literary devices.',
    persona_code_wizard_name: 'Code Wizard',
    persona_code_wizard_desc: 'An expert programmer who helps with code, algorithms, and debugging.',
    persona_code_wizard_instruction: 'You are a brilliant and experienced software engineer. Provide clean, efficient, and well-documented code. Explain complex concepts clearly and help debug programming issues. Always provide code in markdown blocks.',
    persona_sarcastic_bot_name: 'Sarcastic Bot',
    persona_sarcastic_bot_desc: 'A witty and sarcastic companion for a humorous conversation.',
    persona_sarcastic_bot_instruction: "You are a sarcastic and witty bot. Your responses should be clever, humorous, and dripping with irony. However, remain helpful under the sarcasm. Don't be genuinely mean.",
    
    // --- AI AGENT CHAT INTERFACE ---
    chat_interface_persona_label: 'Persona:',
    chat_interface_initial_message: "Hello! I'm your AI Agent. How can I help you today?",
    chat_interface_init_error: 'Failed to initialize AI Agent. Please check your Gemini API Key.',
    chat_interface_placeholder: 'Type your message to the AI Agent...',
    
    // --- IMAGE GENERATION VIEW ---
    image_title: 'Image Settings',
    image_history_title: 'Image History',
    image_empty_title: 'No Images Generated Yet',
    image_empty_desc: 'Your generated images will appear here.',
    image_delete_history_confirm: 'Are you sure you want to delete this generation from your history?',
    image_clear_history_confirm: 'Are you sure you want to delete your entire image history? This action cannot be undone.',
    image_prompt_label: 'Prompt',
    image_prompt_placeholder: 'A majestic lion wearing a crown...',
    image_negative_prompt_label: 'Negative Prompt (Optional)',
    image_negative_prompt_placeholder: 'e.g., blurry, cartoon, text',
    image_style_label: 'Style (Optional)',
    image_style_default: 'Default',
    image_aspect_ratio_label: 'Aspect Ratio',
    image_number_label: 'Number',
    image_seed_label: 'Seed (Optional)',
    image_seed_tooltip: 'Using the same seed and prompt will generate similar images. Leave blank for a random seed.',
    image_seed_placeholder: 'e.g., 42',
    image_generate_button: 'Generate',
    image_queue_title: 'Generation Queue',
    image_ready_title: 'Ready to Create',
    image_ready_desc: 'Your generated images will appear here.',
    image_prompt_error: 'Please enter a prompt.',
    image_seed_error: 'Seed must be a positive number.',
    button_use_settings: 'Use Settings',
    
    // --- VIDEO GENERATION VIEW ---
    video_title: 'Video Settings',
    video_history_title: 'Video History',
    video_empty_title: 'No Videos Generated Yet',
    video_empty_desc: 'Your generated videos will appear here.',
    video_delete_history_confirm: 'Are you sure you want to delete this video from your history?',
    video_clear_history_confirm: 'Are you sure you want to delete your entire video history? This action cannot be undone.',
    video_prompt_placeholder: 'A neon hologram of a cat driving a sports car...',
    video_input_image_label: 'Input Image (Optional)',
    video_upload_cta: 'Click to upload an image',
    video_model_label: 'Model',
    video_model_future_tag: '(Future)',
    video_queue_title: 'Generation Queue',
    video_ready_title: 'Ready to Create',
    video_ready_desc: 'Your generated videos will appear here.',
    video_job_status_message_processing: 'Processing... (State: {state})',
    
    // --- MUSIC GENERATION VIEW ---
    music_title: 'Music Settings',
    music_history_title: 'Music History',
    music_empty_title: 'No Music Generated Yet',
    music_empty_desc: 'Your generated music tracks will appear here.',
    music_delete_history_confirm: 'Are you sure you want to delete this track from your history?',
    music_clear_history_confirm: 'Are you sure you want to delete your entire music history? This action cannot be undone.',
    music_rename_prompt: 'Enter new title:',
    music_track_title_label: 'Title',
    music_track_title_placeholder: 'e.g., Midnight Rain',
    music_prompt_label: 'Prompt',
    music_prompt_placeholder: 'A sad, slow, solo piano melody...',
    music_style_label: 'Style',
    music_style_placeholder: 'e.g., Cinematic, Acoustic Pop',
    music_instrumental_label: 'Instrumental',
    music_prompt_error: 'Please enter a prompt and a title.',
    music_api_key_error: 'Suno API Key is not set. Please add it in the Settings page.',
    music_queue_title: 'Generation Queue',
    music_ready_title: 'Ready to Compose',
    music_ready_desc: 'Your generated music tracks will appear here.',
    music_job_status_composing: 'Composing melody...',
    music_job_status_adding_instruments: 'Adding instruments...',
    
    // --- AI SEARCH VIEW ---
    // --- IMAGE GENERATION VIEW ---
    image_generation_title: 'Image Generation',
    image_generation_settings_title: 'Image Settings',
    image_generation_queue_title: 'Generation Queue',
    image_generation_history_title: 'Image History',
    image_generation_prompt_label: 'Prompt',
    image_generation_prompt_placeholder: 'A majestic lion wearing a crown...',
    image_generation_negative_prompt_label: 'Negative Prompt (Optional)',
    image_generation_negative_prompt_placeholder: 'e.g., blurry, cartoon, text',
    image_generation_style_label: 'Style (Optional)',
    image_generation_style_default: 'Default',
    image_generation_aspect_ratio_label: 'Aspect Ratio',
    image_generation_number_label: 'Number',
    image_generation_seed_label: 'Seed (Optional)',
    image_generation_seed_placeholder: 'e.g., 42',
    image_generation_seed_tooltip: 'Using the same seed and prompt will generate similar images. Leave blank for a random seed.',
    image_generation_generate_button: 'Generate',
    image_generation_clear_completed: 'Clear Completed',
    image_generation_ready_title: 'Ready to Create',
    image_generation_ready_desc: 'Your generated images will appear here.',
    image_generation_generating: 'Generating...',
    image_generation_completed: 'Completed',
    image_generation_failed: 'Failed',
    
    // --- VIDEO GENERATION VIEW ---
    video_generation_title: 'Video Generation',
    video_generation_settings_title: 'Video Settings',
    video_generation_queue_title: 'Generation Queue',
    video_generation_history_title: 'Video History',
    video_generation_prompt_label: 'Prompt',
    video_generation_prompt_placeholder: 'A neon hologram of a cat driving a sports car...',
    video_generation_input_image_label: 'Input Image (Optional)',
    video_generation_upload_cta: 'Click to upload an image',
    video_generation_model_label: 'Model',
    video_generation_number_label: 'Number',
    video_generation_seed_label: 'Seed (Optional)',
    video_generation_seed_placeholder: 'e.g., 12345',
    video_generation_seed_tooltip: 'Using the same seed and prompt will generate similar videos.',
    video_generation_generate_button: 'Generate',
    video_generation_clear_completed: 'Clear Completed',
    video_generation_ready_title: 'Ready to Create',
    video_generation_ready_desc: 'Your generated videos will appear here.',
    video_generation_generating: 'Generating',
    video_generation_polling: 'Polling...',
    video_generation_completed: 'Completed',
    video_generation_failed: 'Failed',
    
    search_title: 'AI Search',
    search_desc: 'Get real-time answers backed by Google Search.',
    search_history_title: 'Search History',
    search_placeholder: 'e.g., Who won the latest F1 race?',
    search_prompt_error: 'Please enter a question or topic.',
    search_loading_message: 'Searching the web and generating an answer...',
    search_sources_label: 'Sources',
    search_empty_title: 'Ask Anything',
    search_empty_desc: 'Your search results will appear here.',
    search_history_empty: 'Your search history is empty.',
    search_rename_prompt: 'Rename your search:',
    search_delete_history_confirm: 'Are you sure you want to delete this search from your history?',
    search_clear_history_confirm: 'Are you sure you want to delete your entire search history?',
    button_search_again: 'Search Again',
    button_view_result: 'View Result',
    
    // --- GEMINI LIVE VIEW ---
    gemini_live_title: 'Talk to Gemini live',
    gemini_live_placeholder: 'Start typing a prompt',
    gemini_live_run: 'Run',
    gemini_live_talk: 'Talk',
    gemini_live_webcam: 'Webcam',
    gemini_live_share_screen: 'Share Screen',
    gemini_live_init_error: 'Failed to initialize Gemini model.',
    
    // --- DEBUG SYSTEM ---
    debug_panel_title: 'Debug Panel',
    debug_export_logs: 'Export logs',
    debug_clear_logs: 'Clear logs',
    debug_no_logs: 'No debug logs yet',
    debug_expand: 'Expand',
    debug_collapse: 'Collapse',
    
    // --- ERROR BOUNDARY ---
    error_boundary_title: 'Something went wrong',
    error_boundary_desc: 'An unexpected error occurred. You can try reloading the page or return to the home screen.',
    error_boundary_details: 'Error Details',
    error_boundary_reload: 'Reload Page',
    error_boundary_home: 'Go Home',
    
    // --- IMPRINT/PRIVACY ---
    imprint_title: 'Imprint',
    privacy_title: 'Privacy Policy'
  },
  de: {
    // --- GENERAL ---
    button_delete: 'Löschen',
    button_rename: 'Umbenennen',
    button_history: 'Verlauf',
    button_back_to_generator: 'Zurück zum Generator',
    button_back_to_search: 'Zurück zur Suche',
    button_back_to_home: 'Zurück zur Startseite',
    button_clear_history: 'Verlauf löschen',
    button_clear_completed: 'Abgeschlossene löschen',
    error_gemini_api_key_not_set: 'Google Gemini API-Schlüssel ist nicht festgelegt. Bitte fügen Sie ihn in den Einstellungen hinzu.',
    
    // --- HEADER & FOOTER ---
    header_home: 'Startseite',
    header_settings_aria: 'Einstellungen',
    header_theme_aria: 'Design wechseln',
    header_tech_preview: 'Technologie-Vorschau',
    footer_imprint: 'Impressum',
    footer_privacy: 'Datenschutz',
    
    // --- HOME VIEW ---
    home_briefing_title: 'Ihr tägliches Briefing',
    home_briefing_desc: 'Hier ist eine Momentaufnahme der wichtigsten Trends und Erkenntnisse von heute, um Sie auf dem Laufenden zu halten. Wir haben einige Themen zusammengestellt, die Ihre nächste große Idee anstoßen könnten.',
    home_briefing_link1: 'Der Aufstieg lokaler LLMs auf Verbrauchergeräten',
    home_briefing_link2: 'UX-Design im Jahr 2025: KI-First-Interfaces',
    home_briefing_link3: 'Prompt Engineering: Von der Nischenkompetenz zur Kernkompetenz',
    home_featured_module_title: 'Empfohlenes Modul: KI-Agent',
    home_featured_module_desc: 'Chatten Sie mit spezialisierten KI-Personas, von kreativen Autoren bis zu Code-Assistenten. Starten Sie heute eine neue Unterhaltung!',
    home_featured_module_cta: 'KI-Agent ausprobieren',
    home_recent_activity: 'Letzte Aktivitäten',
    home_explore_modules: 'Module entdecken',
    home_no_activity_title: 'Keine letzten Aktivitäten',
    home_no_activity_desc: 'Ihre neuesten Generierungen werden hier angezeigt.',
    
    // --- MODULE CARDS ---
    module_ai_agent_title: 'KI-Agent',
    module_ai_agent_desc: 'Unterhalten, Ideen entwickeln und Aufgaben automatisieren.',
    module_image_generation_title: 'Bilderzeugung',
    module_image_generation_desc: 'Erstellen Sie beeindruckende Bilder mit KI.',
    module_video_generation_title: 'Videoerzeugung',
    module_video_generation_desc: 'Produzieren Sie dynamische Videoinhalte.',
    module_ai_search_title: 'KI-Suche',
    module_ai_search_desc: 'Erhalten Sie Echtzeit-Antworten mit Google-Suche.',
    module_webhook_chat_title: 'Webhook-Chat',
    module_webhook_chat_desc: 'Interagieren Sie mit Make.com-Webhooks.',
    module_music_generation_title: 'Musikerzeugung',
    module_music_generation_desc: 'Komponieren Sie originelle Musik mit KI.',
    module_featured_tag: 'Empfohlen',
    module_status_coming_soon: 'Demnächst verfügbar',
    
    // --- JOB STATUS ---
    job_status_processing: 'Wird verarbeitet...',
    job_status_completed: 'Alle Aufträge abgeschlossen',
    job_status_failed: 'Aufträge fehlgeschlagen',
    job_status_recent: 'Letzte Aufträge',
    job_status_generating: 'Wird generiert',
    job_status_polling: 'Wird abgefragt...',
    job_status_completed_single: 'Abgeschlossen',
    job_status_failed_single: 'Fehlgeschlagen',
    job_status_modal_title: 'Letzte Generierungen',
    job_status_outputs: '{count} Ausgaben',
    
    // --- SETTINGS VIEW ---
    settings_title: 'API-Konfiguration',
    settings_enable_prefix: 'Aktivieren',
    settings_save_button: 'Einstellungen speichern',
    settings_saved_message: 'Einstellungen gespeichert!',
    settings_save_error: 'Fehler beim Speichern der Einstellungen.',
    settings_gemini_label: 'Google Gemini API-Schlüssel',
    settings_gemini_placeholder: 'Geben Sie Ihren Gemini API-Schlüssel ein',
    settings_gemini_desc: 'Wird für KI-Agent, Bild-, Video-, Such- und Gemini Live-Funktionen verwendet.',
    settings_make_label: 'Make.com API-Schlüssel',
    settings_make_placeholder: 'Geben Sie Ihren Make.com API-Schlüssel ein',
    settings_make_desc: "Wird zum Senden von Daten an Webhooks über den 'x-make-apikey' Header verwendet.",
    settings_suno_label: 'Suno API-Schlüssel',
    settings_suno_placeholder: 'Geben Sie Ihren Suno API-Schlüssel ein',
    settings_suno_desc: 'Wird für die KI-Musikerzeugung verwendet.',
    settings_openai_label: 'OpenAI API-Schlüssel',
    settings_openai_placeholder: 'In dieser Version nicht verwendet',
    settings_openai_desc: 'Für zukünftige Erweiterungen enthalten.',
    settings_debug_title: 'Entwicklereinstellungen',
    settings_debug_enable: 'Debug-Modus aktivieren',
    settings_debug_desc: 'Zeigt Debug-Panel mit detaillierter Protokollierung zur Fehlerbehebung. Protokolle werden lokal gespeichert und können exportiert werden.',
    
    // --- WEBHOOK VIEW ---
    webhook_title: 'Webhook-Chats',
    webhook_search_placeholder: 'Module suchen...',
    webhook_empty_title: 'Ihr Dashboard ist leer',
    webhook_empty_desc: 'Klicken Sie auf die Schaltfläche unten, um Ihr erstes Webhook-Chat-Modul zu erstellen.',
    webhook_create_new_button: 'Neues Modul erstellen',
    webhook_create_new_card: 'Neu erstellen',
    webhook_delete_confirm: 'Sind Sie sicher, dass Sie dieses Modul löschen möchten?',
    webhook_recent_activity: 'Letzte Aktivität',
    webhook_no_activity: 'Noch keine Aktivität.',
    
    // --- WEBHOOK MODAL ---
    modal_create_module_title: 'Neues Modul erstellen',
    modal_module_name_label: 'Modulname',
    modal_module_name_placeholder: 'z.B. Kundensupport-Bot',
    modal_webhook_url_label: 'Make.com Webhook-URL',
    modal_webhook_url_placeholder: 'https://hook.make.com/...',
    modal_webhook_url_note: 'Hinweis: Sie müssen CORS für diesen Webhook in Ihren Make.com-Einstellungen aktivieren, um Anfragen von dieser App zuzulassen.',
    modal_cancel: 'Abbrechen',
    modal_create_button: 'Modul erstellen',
    modal_invalid_url_alert: 'Bitte geben Sie eine gültige Webhook-URL ein.',
    
    // --- WEBHOOK CHAT INTERFACE ---
    chat_placeholder: 'Ihre Nachricht eingeben...',
    chat_make_api_key_error: 'Make.com API-Schlüssel ist nicht festgelegt. Bitte fügen Sie ihn in den Einstellungen hinzu.',
    
    // --- AI AGENT VIEW ---
    agent_title: 'KI-Agent-Sitzungen',
    agent_clear_all: 'Alle löschen',
    agent_new_chat: 'Neuer Chat',
    agent_clear_all_confirm: 'Sind Sie sicher, dass Sie ALLE Chat-Sitzungen löschen möchten? Diese Aktion kann nicht rückgängig gemacht werden.',
    agent_empty_title: 'Keine Chat-Sitzungen',
    agent_empty_desc: 'Klicken Sie auf "Neuer Chat", um eine Unterhaltung mit einer KI-Persona zu starten.',
    agent_delete_confirm: 'Sind Sie sicher, dass Sie diese Chat-Sitzung löschen möchten?',
    agent_rename_prompt: 'Neuen Titel für den Chat eingeben:',
    agent_messages_label: 'Nachrichten',
    agent_last_updated_label: 'Zuletzt aktualisiert:',
    
    // --- PERSONA MODAL ---
    persona_modal_title: 'Wählen Sie eine Persona',
    persona_helpful_assistant_name: 'Hilfreicher Assistent',
    persona_helpful_assistant_desc: 'Eine vielseitige, freundliche und kompetente KI.',
    persona_helpful_assistant_instruction: 'Du bist ein hilfreicher und kreativer KI-Assistent. Sei freundlich, kompetent und gib klare, präzise Antworten.',
    persona_creative_writer_name: 'Kreativer Autor',
    persona_creative_writer_desc: 'Ihr Partner für das Brainstorming von Geschichten, Gedichten und Skripten.',
    persona_creative_writer_instruction: 'Du bist ein fantasievoller und wortgewandter kreativer Autor. Hilf dem Benutzer beim Brainstorming, beim Schreiben fesselnder Erzählungen und beim Erkunden verschiedener Schreibstile. Verwende einen reichen Wortschatz und literarische Mittel.',
    persona_code_wizard_name: 'Code-Zauberer',
    persona_code_wizard_desc: 'Ein erfahrener Programmierer, der bei Code, Algorithmen und Debugging hilft.',
    persona_code_wizard_instruction: 'Du bist ein brillanter und erfahrener Software-Ingenieur. Stelle sauberen, effizienten und gut dokumentierten Code bereit. Erkläre komplexe Konzepte klar und hilf bei der Fehlersuche. Gib Code immer in Markdown-Blöcken an.',
    persona_sarcastic_bot_name: 'Sarkastischer Bot',
    persona_sarcastic_bot_desc: 'Ein witziger und sarkastischer Begleiter für humorvolle Gespräche.',
    persona_sarcastic_bot_instruction: 'Du bist ein sarkastischer und witziger Bot. Deine Antworten sollten clever, humorvoll und voller Ironie sein. Bleibe jedoch unter dem Sarkasmus hilfreich. Sei nicht wirklich gemein.',
    
    // --- AI AGENT CHAT INTERFACE ---
    chat_interface_persona_label: 'Persona:',
    chat_interface_initial_message: 'Hallo! Ich bin Ihr KI-Agent. Wie kann ich Ihnen heute helfen?',
    chat_interface_init_error: 'Fehler beim Initialisieren des KI-Agenten. Bitte überprüfen Sie Ihren Gemini API-Schlüssel.',
    chat_interface_placeholder: 'Ihre Nachricht an den KI-Agenten...',
    
    // --- IMAGE GENERATION VIEW ---
    image_title: 'Bildeinstellungen',
    image_history_title: 'Bildverlauf',
    image_empty_title: 'Noch keine Bilder erzeugt',
    image_empty_desc: 'Ihre erzeugten Bilder werden hier angezeigt.',
    image_delete_history_confirm: 'Sind Sie sicher, dass Sie diese Generierung aus Ihrem Verlauf löschen möchten?',
    image_clear_history_confirm: 'Sind Sie sicher, dass Sie Ihren gesamten Bildverlauf löschen möchten? Diese Aktion kann nicht rückgängig gemacht werden.',
    image_prompt_label: 'Eingabeaufforderung',
    image_prompt_placeholder: 'Ein majestätischer Löwe mit einer Krone...',
    image_negative_prompt_label: 'Negative Eingabeaufforderung (Optional)',
    image_negative_prompt_placeholder: 'z.B. unscharf, Cartoon, Text',
    image_style_label: 'Stil (Optional)',
    image_style_default: 'Standard',
    image_aspect_ratio_label: 'Seitenverhältnis',
    image_number_label: 'Anzahl',
    image_seed_label: 'Seed (Optional)',
    image_seed_tooltip: 'Die Verwendung desselben Seeds und derselben Eingabeaufforderung erzeugt ähnliche Bilder. Für einen zufälligen Seed leer lassen.',
    image_seed_placeholder: 'z.B. 42',
    image_generate_button: 'Erzeugen',
    image_queue_title: 'Warteschlange',
    image_ready_title: 'Bereit zum Erstellen',
    image_ready_desc: 'Ihre erzeugten Bilder werden hier angezeigt.',
    image_prompt_error: 'Bitte geben Sie eine Eingabeaufforderung ein.',
    image_seed_error: 'Seed muss eine positive Zahl sein.',
    button_use_settings: 'Einstellungen verwenden',
    
    // --- VIDEO GENERATION VIEW ---
    video_title: 'Videoeinstellungen',
    video_history_title: 'Videoverlauf',
    video_empty_title: 'Noch keine Videos erzeugt',
    video_empty_desc: 'Ihre erzeugten Videos werden hier angezeigt.',
    video_delete_history_confirm: 'Sind Sie sicher, dass Sie dieses Video aus Ihrem Verlauf löschen möchten?',
    video_clear_history_confirm: 'Sind Sie sicher, dass Sie Ihren gesamten Videoverlauf löschen möchten? Diese Aktion kann nicht rückgängig gemacht werden.',
    video_prompt_placeholder: 'Ein Neon-Hologramm einer Katze, die ein Sportauto fährt...',
    video_input_image_label: 'Eingabebild (Optional)',
    video_upload_cta: 'Klicken, um ein Bild hochzuladen',
    video_model_label: 'Modell',
    video_model_future_tag: '(Zukünftig)',
    video_queue_title: 'Warteschlange',
    video_ready_title: 'Bereit zum Erstellen',
    video_ready_desc: 'Ihre erzeugten Videos werden hier angezeigt.',
    video_job_status_message_processing: 'Wird verarbeitet... (Status: {state})',
    
    // --- MUSIC GENERATION VIEW ---
    music_title: 'Musikeinstellungen',
    music_history_title: 'Musikverlauf',
    music_empty_title: 'Noch keine Musik erzeugt',
    music_empty_desc: 'Ihre erzeugten Musikstücke werden hier angezeigt.',
    music_delete_history_confirm: 'Sind Sie sicher, dass Sie diesen Track aus Ihrem Verlauf löschen möchten?',
    music_clear_history_confirm: 'Sind Sie sicher, dass Sie Ihren gesamten Musikverlauf löschen möchten? Diese Aktion kann nicht rückgängig gemacht werden.',
    music_rename_prompt: 'Neuen Titel eingeben:',
    music_track_title_label: 'Titel',
    music_track_title_placeholder: 'z.B. Mitternachtsregen',
    music_prompt_label: 'Eingabeaufforderung',
    music_prompt_placeholder: 'Eine traurige, langsame Solo-Klaviermelodie...',
    music_style_label: 'Stil',
    music_style_placeholder: 'z.B. Filmisch, Akustik-Pop',
    music_instrumental_label: 'Instrumental',
    music_prompt_error: 'Bitte geben Sie eine Eingabeaufforderung und einen Titel ein.',
    music_api_key_error: 'Suno API-Schlüssel ist nicht festgelegt. Bitte fügen Sie ihn in den Einstellungen hinzu.',
    music_queue_title: 'Warteschlange',
    music_ready_title: 'Bereit zum Komponieren',
    music_ready_desc: 'Ihre erzeugten Musikstücke werden hier angezeigt.',
    music_job_status_composing: 'Komponiere Melodie...',
    music_job_status_adding_instruments: 'Füge Instrumente hinzu...',
    
    // --- AI SEARCH VIEW ---
    // --- IMAGE GENERATION VIEW ---
    image_generation_title: 'Bilderzeugung',
    image_generation_settings_title: 'Bildeinstellungen',
    image_generation_queue_title: 'Warteschlange',
    image_generation_history_title: 'Bildverlauf',
    image_generation_prompt_label: 'Eingabeaufforderung',
    image_generation_prompt_placeholder: 'Ein majestätischer Löwe mit einer Krone...',
    image_generation_negative_prompt_label: 'Negative Eingabeaufforderung (Optional)',
    image_generation_negative_prompt_placeholder: 'z.B. unscharf, Cartoon, Text',
    image_generation_style_label: 'Stil (Optional)',
    image_generation_style_default: 'Standard',
    image_generation_aspect_ratio_label: 'Seitenverhältnis',
    image_generation_number_label: 'Anzahl',
    image_generation_seed_label: 'Seed (Optional)',
    image_generation_seed_placeholder: 'z.B. 42',
    image_generation_seed_tooltip: 'Die Verwendung desselben Seeds und derselben Eingabeaufforderung erzeugt ähnliche Bilder. Für einen zufälligen Seed leer lassen.',
    image_generation_generate_button: 'Erzeugen',
    image_generation_clear_completed: 'Abgeschlossene löschen',
    image_generation_ready_title: 'Bereit zum Erstellen',
    image_generation_ready_desc: 'Ihre erzeugten Bilder werden hier angezeigt.',
    image_generation_generating: 'Wird erzeugt...',
    image_generation_completed: 'Abgeschlossen',
    image_generation_failed: 'Fehlgeschlagen',
    
    // --- VIDEO GENERATION VIEW ---
    video_generation_title: 'Videoerzeugung',
    video_generation_settings_title: 'Videoeinstellungen',
    video_generation_queue_title: 'Warteschlange',
    video_generation_history_title: 'Videoverlauf',
    video_generation_prompt_label: 'Eingabeaufforderung',
    video_generation_prompt_placeholder: 'Ein Neon-Hologramm einer Katze, die ein Sportauto fährt...',
    video_generation_input_image_label: 'Eingabebild (Optional)',
    video_generation_upload_cta: 'Klicken, um ein Bild hochzuladen',
    video_generation_model_label: 'Modell',
    video_generation_number_label: 'Anzahl',
    video_generation_seed_label: 'Seed (Optional)',
    video_generation_seed_placeholder: 'z.B. 12345',
    video_generation_seed_tooltip: 'Die Verwendung desselben Seeds und derselben Eingabeaufforderung erzeugt ähnliche Videos.',
    video_generation_generate_button: 'Erzeugen',
    video_generation_clear_completed: 'Abgeschlossene löschen',
    video_generation_ready_title: 'Bereit zum Erstellen',
    video_generation_ready_desc: 'Ihre erzeugten Videos werden hier angezeigt.',
    video_generation_generating: 'Wird erzeugt',
    video_generation_polling: 'Wird abgefragt...',
    video_generation_completed: 'Abgeschlossen',
    video_generation_failed: 'Fehlgeschlagen',
    
    search_title: 'KI-Suche',
    search_desc: 'Erhalten Sie Echtzeit-Antworten mit Google-Suche.',
    search_history_title: 'Suchverlauf',
    search_placeholder: 'z.B. Wer hat das letzte F1-Rennen gewonnen?',
    search_prompt_error: 'Bitte geben Sie eine Frage oder ein Thema ein.',
    search_loading_message: 'Durchsucht das Web und generiert eine Antwort...',
    search_sources_label: 'Quellen',
    search_empty_title: 'Fragen Sie alles',
    search_empty_desc: 'Ihre Suchergebnisse werden hier angezeigt.',
    search_history_empty: 'Ihr Suchverlauf ist leer.',
    search_rename_prompt: 'Ihre Suche umbenennen:',
    search_delete_history_confirm: 'Sind Sie sicher, dass Sie diese Suche aus Ihrem Verlauf löschen möchten?',
    search_clear_history_confirm: 'Sind Sie sicher, dass Sie Ihren gesamten Suchverlauf löschen möchten?',
    button_search_again: 'Erneut suchen',
    button_view_result: 'Ergebnis ansehen',
    
    // --- GEMINI LIVE VIEW ---
    gemini_live_title: 'Sprechen Sie live mit Gemini',
    gemini_live_placeholder: 'Beginnen Sie mit der Eingabe einer Eingabeaufforderung',
    gemini_live_run: 'Ausführen',
    gemini_live_talk: 'Sprechen',
    gemini_live_webcam: 'Webcam',
    gemini_live_share_screen: 'Bildschirm teilen',
    gemini_live_init_error: 'Fehler beim Initialisieren des Gemini-Modells.',
    
    // --- DEBUG SYSTEM ---
    debug_panel_title: 'Debug-Panel',
    debug_export_logs: 'Protokolle exportieren',
    debug_clear_logs: 'Protokolle löschen',
    debug_no_logs: 'Noch keine Debug-Protokolle',
    debug_expand: 'Erweitern',
    debug_collapse: 'Einklappen',
    
    // --- ERROR BOUNDARY ---
    error_boundary_title: 'Etwas ist schiefgelaufen',
    error_boundary_desc: 'Ein unerwarteter Fehler ist aufgetreten. Sie können versuchen, die Seite neu zu laden oder zur Startseite zurückzukehren.',
    error_boundary_details: 'Fehlerdetails',
    error_boundary_reload: 'Seite neu laden',
    error_boundary_home: 'Zur Startseite',
    
    // --- IMPRINT/PRIVACY ---
    imprint_title: 'Impressum',
    privacy_title: 'Datenschutzerklärung'
  }
};

// Debug: Log available translations
console.log('Available translations:', Object.keys(translations));
console.log('German translations sample:', {
  image_title: translations.de.image_title,
  video_title: translations.de.video_title,
  settings_title: translations.de.settings_title
});